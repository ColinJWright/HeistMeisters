...
func update_pointer_position(objective_number):
	...
	$Tween.interpolate_property(pointer, "position", pointer.position, place.position, 1, Tween.TRANS_SINE, Tween.EASE_IN_OUT)
	$Tween.start()
	$MessageSound.play()
...
func _on_BoxObjective_body_entered(body):
